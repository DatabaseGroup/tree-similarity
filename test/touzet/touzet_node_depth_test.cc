...
int main() {
  ...
  std::ifstream test_cases_file("touzet_node_depth_test_data.txt");
  ...
  for (std::string line; std::getline( test_cases_file, line);) {
    if (line[0] == '#') {
      ...
      std::string computed_results = vector_to_string(touzet_test_items.t1_depth);

      if (correct_result != computed_results) {
        std::cerr << "Incorrect nodes depth result: " << computed_results << " instead of " << correct_result << std::endl;
