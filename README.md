...
- [`valgrind`](#references) in version >= 3.10.1 (**not mandatory** but required if one wants to use
`make check-dynamic`/`make check`)
- [`cppcheck`](#references) in version >= 1.74 (**not mandatory** but required if one wants to use
...
~~~
mkdir -p build && cd build/ && CXX=clang++ cmake .. && make
~~~
