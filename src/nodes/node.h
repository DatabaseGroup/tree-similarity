...
void Node::add_edge (char colour) {
  edges_.push_back(colour);
}

void Node::add_edge_at(char colour, int position) {
  std::vector<char>::iterator it = edges_.begin();
  edges_.insert(it + position, colour);
}

void Node::remove_edge (int position) {
  edges_.erase(edges_.begin() + position);
}

void Node::set_edge_colour (int position, char colour) {
  edges_[position] = colour;
}

char Node::get_edge_colour (int position) {
  return edges_[position];
}

int Node::get_edge_colours_count (char first, char second) {
  int counter = 0;    
...
void Node::set_colour (char colour) {
  colour_ = colour;
}
